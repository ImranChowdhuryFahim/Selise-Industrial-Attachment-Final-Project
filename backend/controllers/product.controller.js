...
module.exports = {
  ...
  getTransformedProducts: async (req, res, next) => {
    ...
    const products = await ProductSchema.find().limit(perPage)
    .skip(perPage * page)
    .sort(sortPayload)
  ...
  createProduct: async (req, res, next) => {
    ...
    const nameExist = await ProductSchema.findOne({productName:req.body.productName})
    const shortCodeExist = await ProductSchema.findOne({productShortCode:req.body.productShortCode})
  ...
  updateProduct: async (req, res, next) => {
    ...
    ProductSchema.findByIdAndUpdate(req.body._id, {...req.body}, {upsert:true,new:true})
      .then((product) => {
        return res.json({
          isSuccessful: true,
